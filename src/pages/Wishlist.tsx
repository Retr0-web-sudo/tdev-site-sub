...
import { Heart, ArrowRight, Lock, Sparkles, Star, Crown } from "lucide-react";
...
const Wishlist = () => {
  ...
  const lockedItems = items.filter(item => {
    const tier = (item as any).tier || "essentials";
      ...
      <main className="section-padding pt-24 md:pt-28 pb-16 sm:pb-20">
        ...
        {items.length === 0 ? (
          <motion.div className="text-center py-16" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            ...
            <Link to="/shop" className="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-accent text-accent-foreground text-[11px] tracking-[0.15em] uppercase font-body font-medium hover:bg-accent/90 transition-all">
              ...
              <motion.div className="bg-accent/10 border border-accent/30 rounded-xl p-5 flex flex-col sm:flex-row items-center justify-between gap-4" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-body text-xs text-muted-foreground">Upgrade to {tierLabels[nextTier]} to unlock them all.</p>
                  </div>
                </div>
                    ...
                    return (
                      <motion.div key={item.id} className="group bg-card/50 border border-border/40 rounded-xl overflow-hidden hover:border-accent/30 transition-all" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                        <div className="aspect-[3/4] bg-secondary/30 relative overflow-hidden">
                          ...
                          <button onClick={() => removeFromWishlist(item.id)} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-background/80 backdrop-blur-sm flex items-center justify-center text-red-400 hover:bg-red-500/20 transition-colors">
                            <Heart size={12} fill="currentColor" />
                          </button>
                          <span className={`absolute top-2 left-2 px-2 py-0.5 rounded text-[9px] font-body tracking-wider uppercase bg-background/80 backdrop-blur-sm ${tierColors[tier]}`}>
                            {tierLabels[tier] || tier}
                          </span>
                        </div>
                        <div className="p-3">
                          <p className="text-[10px] text-muted-foreground font-body uppercase tracking-wider">{item.brand || "TDEV"}</p>
                          <Link to={`/product/${item.slug}`} className="block text-sm font-body font-medium text-foreground truncate hover:text-accent transition-colors">{item.name}</Link>
                    ...
                    return (
                      <motion.div key={item.id} className="group relative bg-card/50 border border-border/40 rounded-xl overflow-hidden opacity-60" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 0.6, y: 0 }} transition={{ delay: i * 0.05 }}>
                        <div className="aspect-[3/4] bg-secondary/30 relative overflow-hidden">
                          ...
                          <span className={`absolute top-2 left-2 px-2 py-0.5 rounded text-[9px] font-body tracking-wider uppercase bg-background/80 backdrop-blur-sm ${tierColors[tier]}`}>
                            <Lock size={8} className="inline mr-1" />{tierLabels[tier] || tier}
                          </span>
                        </div>
                        <div className="p-3">
                          <p className="text-[10px] text-muted-foreground font-body uppercase tracking-wider">{item.brand || "TDEV"}</p>
